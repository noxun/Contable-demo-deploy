...
export const BalanceGeneralPreview = ({
  data,
  dateRange,
  currentLevel,
  inSus = false,
      ...
      <Fragment key={item.code}>
        <tr className="hover:bg-blue-500/20 dark:hover:bg-blue-500/40">
          ...
          <td
            ...
            className={`${
              currentLevel === level ? "font-normal" : "font-semibold"
            }`}
          >
            {item.level === 5 || item.level === 6 ? (
        ...
        {item.itemsChild.length > 0 &&
          item.itemsChild.map((child) => renderItem(child, level + 1))}
            ...
            {item.description === "PASIVOS" && (
              <>
                ...
                <tr className="font-bold bg-gray-50 dark:bg-gray-800">
                  ...
                  <td colSpan={3} className="text-end">
                    {formatNumber(data.totalPassive)}
                  </td>
                </tr>
                <tr className="h-6"></tr>
              </>
              ...
              <>
                ...
                <tr className="font-bold border-gray-300 dark:bg-gray-800 bg-gray-50">
                  ...
                  <td colSpan={3} className="text-right pr-4">
                    {formatNumber(data.totalHeritage)}
                  </td>
                </tr>
                ...
                <tr className="font-bold border-gray-400 dark:bg-gray-800 bg-gray-50">
                  ...
                  <td colSpan={3} className="text-right pr-4">
                    {formatNumber(data.liabilityEquityResult)}
                  </td>
                </tr>
              </>
            )}
  ...
  return (
    <>
      ...
      >
        ...
        <tbody>{data.items.map((item) => renderItem(item))}</tbody>
      </table>
    </>
  );
};
